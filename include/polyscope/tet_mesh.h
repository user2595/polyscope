...
struct EdgePt {
  ...
  EdgePt(float bary_, size_t src_, size_t dst_) : bary(bary_), src(src_), dst(dst_){};
};
...
public:
  ...
  void fillGeometryBuffersPick(render::ShaderProgram& p);

  // = Visualization settings
  PersistentValue<glm::vec3> tetColor;
  ...
  template <class T>
  TetVertexScalarQuantity* addVertexScalarQuantity(std::string name, const T& data,
  ...
  template <class T>
  TetFaceScalarQuantity* addFaceScalarQuantity(std::string name, const T& data, DataType type = DataType::STANDARD) {
  ...
  template <class T>
  TetVertexVectorQuantity* addVertexVectorQuantity(std::string name, const T& vectors,
  ...
  template <class T>
  TetFaceVectorQuantity* addFaceVectorQuantity(std::string name, const T& vectors,
                                               VectorType vectorType = VectorType::STANDARD) {
  ...
  template <class T>
  TetTetVectorQuantity* addTetVectorQuantity(std::string name, const T& vectors,
                                             VectorType vectorType = VectorType::STANDARD) {
    validateSize(vectors, tets.size(), "tet vector quantity " + name);
    return addTetVectorQuantityImpl(name, standardizeVectorArray<glm::vec3, 3>(vectors), vectorType);
  }


  TetVertexScalarQuantity* addVertexScalarQuantityImpl(std::string name, const std::vector<double>& data,
                                                       DataType type);
  TetFaceScalarQuantity* addFaceScalarQuantityImpl(std::string name, const std::vector<double>& data, DataType type);
  TetVertexVectorQuantity* addVertexVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                       VectorType vectorType);
  TetFaceVectorQuantity* addFaceVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                   VectorType vectorType);

  TetTetVectorQuantity* addTetVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                 VectorType vectorType);
  // === Member variables ===
  static const std::string structureTypeName;

  // Flat or smooth shading
  TetMesh* setSmoothShade(bool isSmooth);
  bool isSmoothShade();

  // Color of the mesh
  TetMesh* setTetColor(glm::vec3 val);
  glm::vec3 getTetColor();

  // Color of edges
  TetMesh* setEdgeColor(glm::vec3 val);
  glm::vec3 getEdgeColor();

  // Material
  TetMesh* setMaterial(std::string name);
  ...
  TetMesh* setEdgeWidth(double newVal);
  double getEdgeWidth();
};
...
TetMesh* registerTetMesh(std::string name, const V& vertexPositions, const T& tetIndices, const S& neigh,
                         bool replaceIfPresent = true) {
  TetMesh* t = new TetMesh(name, standardizeVectorArray<glm::vec3, 3>(vertexPositions),
                           standardizeNestedList<size_t, T>(tetIndices), standardizeArray<int, S>(neigh));
  bool success = registerStructure(t);
  if (!success) {
    safeDelete(t);
  }
...
inline TetMesh* getTetMesh(std::string name = "") {
  return dynamic_cast<TetMesh*>(getStructure(TetMesh::structureTypeName, name));
}
