...
TetMesh::TetMesh(std::string name_)
    : QuantityStructure<TetMesh>(name_, structureTypeName), name(name_),
      tetColor(uniquePrefix() + "tetColor", getNextUniqueColor()),
...
TetMesh::TetMesh(std::string name_, std::vector<glm::vec3> vertices_, std::vector<std::vector<size_t>> tets_)
    : QuantityStructure<TetMesh>(name_, structureTypeName), vertices(vertices_), name(name_),
      tetColor(uniquePrefix() + "tetColor", getNextUniqueColor()),
      edgeColor(uniquePrefix() + "edgeColor", glm::vec3{0., 0., 0.}), material(uniquePrefix() + "material", "clay"),
      edgeWidth(uniquePrefix() + "edgeWidth", 0.) {
...
TetMesh::TetMesh(std::string name_, std::vector<glm::vec3> vertices_, std::vector<std::vector<size_t>> tets_,
    ...
    : QuantityStructure<TetMesh>(name_, structureTypeName), vertices(vertices_), tetNeighbors(tetNeighbors_),
      name(name_), tetColor(uniquePrefix() + "tetColor", getNextUniqueColor()),
      edgeColor(uniquePrefix() + "edgeColor", glm::vec3{0., 0., 0.}), material(uniquePrefix() + "material", "clay"),
      edgeWidth(uniquePrefix() + "edgeWidth", 0.) {
...
void TetMesh::draw() {
  ...
  if (dominantQuantity == nullptr) {
    ...
    program->setUniform("u_basecolor", tetColor.get());
...
std::vector<EdgePt> TetMesh::clipFace(const std::array<size_t, 3>& face, const std::array<double, 3>& fn) {
  ...
  if (numGeqZero == 0) {
    ...
    return std::vector<EdgePt>{EdgePt{1, face[0], face[0]}, EdgePt{1, face[1], face[1]}, EdgePt{1, face[2], face[2]}};
  }
}
...
std::vector<EdgePt> TetMesh::clipTet(const std::array<size_t, 4>& tet, const std::array<double, 4>& fn) {
  ...
  if (pts.size() == 4 && glm::dot(glm::cross(q[1] - q[0], q[2] - q[0]), glm::cross(q[2] - q[0], q[3] - q[0])) < 0.) {
...
glm::vec3 TetMesh::pos(EdgePt e) { return e.bary * vertices[e.src] + (1.f - e.bary) * vertices[e.dst]; }

double TetMesh::interp(EdgePt e, const std::vector<double>& f) { return e.bary * f[e.src] + (1. - e.bary) * f[e.dst]; }
...
void TetMesh::precomputeVisibleGeometry() {
  ...
  if (sliceThroughTets) {
    auto distFn = [&](size_t iV) { return sliceDist - glm::dot(sliceNormal, vertices[iV]); };
                       ...
                       std::vector<double>& T)                    // out: intersection times
{
  ...
  if (q.size() == 4 && dot(cross(q[1] - q[0], q[2] - q[0]), cross(q[2] - q[0], q[3] - q[0])) < 0.) {
    std::swap(q[2], q[3]);
    std::swap(e[2], e[3]);
    std::swap(T[2], T[3]);
  }
...
void TetMesh::fillGeometryBuffers(render::ShaderProgram& p) {
...
void TetMesh::fillGeometryBuffersWireframe(render::ShaderProgram& p) {
  std::vector<glm::vec3> positions;
  std::vector<glm::vec3> normals;
  std::vector<glm::vec3> bcoord;
...
void TetMesh::fillGeometryBuffersPick(render::ShaderProgram& p) {
  // Get element indices
...
void TetMesh::buildCustomUI() {
  ...
  { // colors
    if (ImGui::ColorEdit3("Color", &tetColor.get()[0], ImGuiColorEditFlags_NoInputs)) setTetColor(tetColor.get());
    ImGui::SameLine();
  }
  ...
  { // Edge width
    ...
    if (ImGui::SliderFloat("Plane theta", &sliceTheta, 0., 2. * PI, "%.5f", 2.)) {
      sliceNormal = glm::vec3{cos(sliceTheta) * cos(slicePhi), sin(slicePhi), sin(sliceTheta) * cos(slicePhi)};
      geometryChanged();
    }
...
TetMesh* TetMesh::setTetColor(glm::vec3 val) {
  tetColor = val;
  requestRedraw();
  return this;
}
glm::vec3 TetMesh::getTetColor() { return tetColor.get(); }

TetMesh* TetMesh::setEdgeColor(glm::vec3 val) {
  edgeColor = val;
  requestRedraw();
  return this;
}
glm::vec3 TetMesh::getEdgeColor() { return edgeColor.get(); }

TetMesh* TetMesh::setMaterial(std::string m) {
  material = m;
  geometryChanged(); // (serves the purpose of re-initializing everything, though this is a bit overkill)
  requestRedraw();
  return this;
}
std::string TetMesh::getMaterial() { return material.get(); }

TetMesh* TetMesh::setEdgeWidth(double newVal) {
  edgeWidth = newVal;
  requestRedraw();
  return this;
}
double TetMesh::getEdgeWidth() { return edgeWidth.get(); }

// === Quantity adders

TetVertexScalarQuantity* TetMesh::addVertexScalarQuantityImpl(std::string name, const std::vector<double>& data,
                                                              DataType type) {
  TetVertexScalarQuantity* q = new TetVertexScalarQuantity(name, data, *this, type);
  addQuantity(q);
  return q;
}

TetFaceScalarQuantity* TetMesh::addFaceScalarQuantityImpl(std::string name, const std::vector<double>& data,
                                                          DataType type) {
  TetFaceScalarQuantity* q = new TetFaceScalarQuantity(name, data, *this, type);
  addQuantity(q);
  return q;
}


TetVertexVectorQuantity* TetMesh::addVertexVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                              VectorType vectorType) {
  TetVertexVectorQuantity* q = new TetVertexVectorQuantity(name, vectors, *this, vectorType);
  addQuantity(q);
  return q;
}

TetFaceVectorQuantity* TetMesh::addFaceVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                          VectorType vectorType) {

  TetFaceVectorQuantity* q = new TetFaceVectorQuantity(name, vectors, *this, vectorType);
  addQuantity(q);
  return q;
}

TetTetVectorQuantity* TetMesh::addTetVectorQuantityImpl(std::string name, const std::vector<glm::vec3>& vectors,
                                                        VectorType vectorType) {

  TetTetVectorQuantity* q = new TetTetVectorQuantity(name, vectors, *this, vectorType);
  addQuantity(q);
  return q;
}

TetMeshQuantity::TetMeshQuantity(std::string name, TetMesh& parentStructure, bool dominates)
